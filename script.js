$(document).ready(function(){
    ...
    $('area').click(function() {
        //event.preventDefault(); // Prevenire il comportamento predefinito del clic
        $("b").removeClass('paragrafo_mappa');
        var areaId = $(this).attr('id'); // Ottieni l'ID dell'area cliccata
        var targetB = $("b#" + areaId); // Seleziona il div con l'ID corrispondente
        targetB.addClass('paragrafo_mappa');
    
        if (targetB.length) { // Verifica che il div esista
            $('html, body').animate({
                scrollTop: targetB.offset().top // Scorri fino alla posizione del div
